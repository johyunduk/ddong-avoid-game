...
캐릭터 목록 화면에서 캐릭터를 선택하면 일러스트가 뜨는데 해당 일러스트에서 목록이 클릭 되는 현상이 있는거 같아. 그리고 x 버튼 버튼 전체가 눌러지는게 아니라 진짜 x 부분만 누를 수 있는거 같아

### Prompt 63

현재 전체 사용자에게 스코르 2만개씩 넣어주려고 하는데 쿼리를 알려줘. user_skor 테이블에 넣으면 될 듯 한데

### Prompt 64

자 그리고 지금 게임 오버시 스코르 정제하는거 스코르 획득량이 너무 적은거 같아 그리고 주간 한도 일단 제거하자
