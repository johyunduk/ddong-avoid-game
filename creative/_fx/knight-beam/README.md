> **폐기됨 (2026-09-06)** — 나이트 검기는 이제 **프레임 시트**를 쓴다.
> `../slash-sheet/README.md` 참고. 여기 있는 정지 텍스처(`sword-beam.png`)는 삭제했다.
> 아래 내용은 **정지 텍스처를 만들 때의 기록**으로 남긴다 — 색상·채도·반투명 처리는
> 다른 이펙트에도 그대로 적용되는 규칙이라 버리지 않는다.
