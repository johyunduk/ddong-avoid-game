...
글리치 본체를 따라다니는 분신은 좀 더 멀리서부터 따라오게 만들어 주고 200점마다 신규 분신이 특수 똥 위치에 생기는거 분신 세개로 해서 특수똥 최대 3개씩 먹도록 수정해줘.

### Prompt 95

분신 딜레이 24 프레임으로 변경하고 특수 똥에 생성되는 분신은 좀 빠르게 사라지게 해줘
...
글리치 특수 효과 지금꺼 말고 좀 더 글리치 스러운게 뭐가 있을까?

### Prompt 98

1번 픽셀 노이즈 버스트가 생기는데 일반똥이 거기에 닿으면 사라지게 만들어줘. 대신 닿으면 노이즈 버스트도 사라지는거야

### Prompt 99

기존 특수 똥 3개 수집하는 분신은 없애줘. 그리고 컬러 픽셀 블록을 검정색 배경에 네온 핑크, 네온 그린으로 신비로운 픽셀 박스 노이즈 같은거로 보이게 해주고 0.4~0.9초 뒤에 소멸하는건 없애줘. 일반똥에 닿은 경우에만 없어지게 해줘

### Prompt 100

픽셀 박스 5개로 바꿔주고 테두리가 네온 핑크, 네온 그린이 아니라 안에 랜덤하게 있어서 뭔가 tv 액정이 뜯어진 느낌 같은 그런거로 해줘

### Prompt 101

아 그리고 200점 마다가 아니라 100점 마다로 바꿔줘

### Prompt 102

박스 크기 좀 줄여주고 0.5초 유지되게 하자.
...
글리치를 따라 다니는 분신이 아니라 혼자 계속 왔다갔다 하게 만들어줄 수 있나?
...
1. Session started with context from previous conversation - memory correction for leaderboard-submit deploy flag
2. Documentation update requested
3. Discussion about preventing direct API score submission attacks
4. Game session system implementation for server-side time validation
5....
